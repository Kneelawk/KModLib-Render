                    ...
                    for (Resource res : entry.getValue()) {
                        try (BufferedReader reader = res.getReader()) {
                            ...
                            keysBuilder.addAll(json.values().stream()
                                .map(id -> RegistryKey.of((RegistryKey<Registry<Object>>) registryKey, id)).toList());
